...
export default {
  ...
  data() {
    return {
      ...
      menuVisible: false,
      menuItemList: [
        { key: "1", icon: "arrow-left", text: "Close left" },
        { key: "2", icon: "arrow-right", text: "Close right" },
  ...
  methods: {
    ...
    remove(key) {
      if (this.pageList.length === 1) {
        return this.$message.warning(
          "This is the last page and can't be closed anymore"
        );
      }
    ...
    closeLeft(pageKey) {
      ...
      if (
        this.pageList.findIndex(item => item.fullPath === this.activePage) ===
        -1
      ) {
        this.activePage = this.pageList[0].fullPath;
        this.$router.push(this.activePage);
